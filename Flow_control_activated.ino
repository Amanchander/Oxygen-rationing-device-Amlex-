// This code is actual code with valve control. 
...
float upper_cutoff_limit = 50;
float lower_cutoff_limit = -50;

float max_slope = 0; // maximum slope
float min_slope = 0; // minimum slope
float slope_iteration = 10; // system will take max/min value of perious 'avg_slope_iteration' samples
int slope_iter_count = 0; 
float alpha = 0.01;

float old_humidity_data = 0;
float exp_humidity_data = 0;
...
{ 
  ...
  Serial.println("LABEL,TIME_STAMP,Time,slope,upper_threshold,lower_threshold,flow_rate,himidity,valve_status,breathing_status");
...
{ 
  ...
  if(initial_time_count >= initial_time_count_limit) // Keep the valve open and wait for some time until the initial_time_count is greater than initial_time_count_limit. 
  {    
...
{  
  ...
  old_humidity_data=exp_humidity_data;
  ...
  { 
   ...
   exp_humidity_data =  exp(0.15*current_humidity_data);
   total=total + exp_humidity_data;
  }

  total=total/cnt;
  exp_humidity_data = total;
  
  time_count++;
       

  slope=(exp_humidity_data - old_humidity_data)/2;
